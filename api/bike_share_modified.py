...
def get_all_stations():
    ...
    for station in stations:
        #v=get_cluster(station.get('STATION_ID'))[0]
        X = np.array([[get_cluster(station.get('STATION_ID'))[0].get('cluster_id'), seasons(curr_time.month),\
             curr_time.weekday(), curr_time.hour]])
        #print("Test all stations",9.get('cluster_id');
        #print("X value: ", X)
        #v=[y[0] for y in X]
        #x=[get_cluster(v.get('STATION_ID'))[0] for v in stations[:9]]
        print(X)     
        demand = model.predict(X)
        #print("Demand value: ",demand)
...
def get_station():
    ...
    if data:
        ...
        #for x in related_stations:
            #print("related_stations array value",x)

        i = 1
        while(i < len(related_stations)):
            if related_stations[i].get('dest_id') == related_stations[0].get('dest_id'):
                print("related_stations_value",related_stations[1].get('dest_id'))
